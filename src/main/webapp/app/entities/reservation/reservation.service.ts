...
export class ReservationService {
  ...
  protected convertDateFromClient(reservation: IReservation): IReservation {
    const copy: IReservation = Object.assign({}, reservation, {
      dateDebut: reservation.dateDebut && reservation.dateDebut.isValid() ? reservation.dateDebut.format(DATE_FORMAT) : undefined,
      dateFin: reservation.dateFin && reservation.dateFin.isValid() ? reservation.dateFin.format(DATE_FORMAT) : undefined,
    });
    return copy;
  }
  ...
  protected convertDateArrayFromServer(res: EntityArrayResponseType): EntityArrayResponseType {
    if (res.body) {
      res.body.forEach((reservation: IReservation) => {
        reservation.dateDebut = reservation.dateDebut ? moment(reservation.dateDebut) : undefined;
        reservation.dateFin = reservation.dateFin ? moment(reservation.dateFin) : undefined;
      });
    }
    return res;
  }
